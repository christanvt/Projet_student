...
class AppCustomAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $providerKey)
    {
        ...
        if (
            in_array('ROLE_ADMIN', $roleNames)
            || in_array('ROLE_TEACHER', $roleNames)
        ) {
            return new RedirectResponse($this->urlGenerator->generate('school_year_index'));
        } elseif (
            in_array('ROLE_STUDENT', $roleNames)
        ) {
            return new RedirectResponse($this->urlGenerator->generate('school_year_show', ['id' => $user->getSchoolYear()->getId(),]));
        } elseif (
            in_array('ROLE_CLIENT', $roleNames)
        ) {
